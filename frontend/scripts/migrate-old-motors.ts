...
function extractDisplacement(name: string): string | undefined {
...
function extractPower(name: string, description: string): string | undefined {
  // Look in name first
...
function extractMileage(description: string): string {
...
function extractYear(name: string, description: string): string | undefined {
...
function determineCondition(name: string, description: string): string {
...
function extractCompatibility(name: string, description: string): string[] {
  const models: string[] = []
...
function createSpecifications(motor: OldMotor): {label: string; value: string}[] {
...
function extractDamageDescription(description: string): string | undefined {
...
export async function migrateOldMotorsToSanity(jsonFilePath: string) {
  try {
    console.log('Starting old motors migration to Sanity...')

    // Read the JSON file
    const jsonData = fs.readFileSync(jsonFilePath, 'utf8')
    const motors: OldMotor[] = JSON.parse(jsonData)
    ...
    for (const motor of motors) {
      try {
        ...
        const staryMotor: SanityStaryMotor = {
          _type: 'staryMotor',
          name: motor.name,
          slug: {
            _type: 'slug',
...
export async function runOldMotorsMigration() {
