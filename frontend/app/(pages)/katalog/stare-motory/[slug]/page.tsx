...
export default async function StaryMotorDetailPage({params}: Props) {
  const {slug} = await params
  const [{data: motor}, {data: settings}] = await Promise.all([
    sanityFetch({query: staryMotorQuery, params: {slug}, stega: false}),
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-6">
          ...
          {motor.mileage && <p className="text-sm text-gray-500">Nájezd: {motor.mileage} km</p>}
          {motor.year && <p className="text-sm text-gray-500">Rok výroby: {motor.year}</p>}
          ...
          {motor.damageDescription && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Popis poškození</h3>
              <p className="text-gray-600">{motor.damageDescription}</p>
            </div>
          )}
          {motor.specifications && motor.specifications.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {motor.specifications.map((spec: any, index: any) => (
