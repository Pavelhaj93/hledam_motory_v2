...
import {repasovanyMotorQuery, repasovaneMotoryPagesSlugs} from '@/sanity/lib/queries'
...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  return {
    title: motor.seo?.metaTitle || `${motor.name} | Repasovaný motor ${motor.brand?.name || ''}`,
    description: motor.seo?.metaDescription || motor.description || '',
    alternates: {
      canonical: `/katalog/repasovane-motory/${slug}`,
    },
    openGraph: {
      title: motor.seo?.metaTitle || `${motor.name} | Repasovaný motor ${motor.brand?.name || ''}`,
      description: motor.seo?.metaDescription || motor.description || '',
...
export default async function RepasovanyMotorDetailPage({params}: Props) {
  ...
  const schemas = productJsonLd({
    ...
    slug,
    categoryPath: 'katalog/repasovane-motory',
    categoryLabel: 'Repasované motory',
  })
  return (
